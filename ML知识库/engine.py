...
class MLKnowledgeBaseSkill:
    ...
    def recognize_task_type(self, user_query: str) -> str:
        """
        识别任务类型（AP-02任务类型体系）
        返回: 答疑 / 评判 / 诊断 / 生成 / 规划 / 审核 / 问题诊断 / 模型选型 / 代码生成
        """
        query_lower = user_query.lower()

        # 问题诊断关键词
        if any(k in query_lower for k in ["为什么", "原因", "问题", "诊断", "根因", "效果差", "表现"]):
            return "问题诊断"
        # 模型选型关键词
        if any(k in query_lower for k in ["选型", "推荐", "模型选择", "用哪个", "哪个好"]):
            return "模型选型"
        # 代码生成关键词
        if any(k in query_lower for k in ["生成代码", "写代码", "代码实现", "帮我写", "怎么写"]):
            return "代码生成"
        # 答疑关键词
        if any(k in query_lower for k in ["原理", "是什么", "概念", "解释", "什么是", "什么意思"]):
            return "答疑"
        # 评判关键词
        if any(k in query_lower for k in ["评估", "评判", "判断", "好不好"]):
            return "评判"
        # 诊断关键词
        if any(k in query_lower for k in ["诊断", "问题定位", "哪里错了"]):
            return "诊断"
        # 生成关键词
        if any(k in query_lower for k in ["生成", "创建", "制作"]):
            return "生成"
        # 审核关键词
        if any(k in query_lower for k in ["审核", "检查", "评审"]):
            return "审核"
        # 规划关键词
        if any(k in query_lower for k in ["规划", "路线", "步骤"]):
            return "规划"
        # 深度学习特定关键词（提高识别精度）
        if any(k in query_lower for k in ["cnn", "卷积", "resnet", "vgg", "alexnet", "densenet", "lenet"]):
            return "答疑"
        if any(k in query_lower for k in ["rnn", "lstm", "gru", "循环神经网络", "长短期记忆"]):
            return "答疑"
        if any(k in query_lower for k in ["transformer", "注意力", "多头注意力", "位置编码"]):
            return "答疑"
