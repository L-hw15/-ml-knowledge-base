...
**领域**：机器学习（Phase A：传统ML / Phase B：深度学习 / Phase C：AI大模型）
**Skill形态**：可挂载到AI Agent的机器学习领域助手
**能力档位**：L3 严格审查档
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.2.0 | 2026-09-16 | Phase C扩展：簇D AutoML与模型压缩（剪枝/量化/知识蒸馏） |
| v1.1.0 | 2026-09-16 | Phase B扩展：簇C经典深度学习（CNN/RNN/Transformer） |
...
| 簇C 经典深度 | **LeNet/AlexNet/VGG/ResNet/DenseNet(CNN)**、**LSTM/GRU(RNN)**、**Transformer/注意力机制/位置编码**、**Adam/Momentum/SGD(优化器)** |
| 簇D AutoML | **模型剪枝(L1/Ln结构化/全局)**、**知识蒸馏(Teacher-Student/软标签/蒸馏温度)**、**模型量化(动态/静态/量化感知训练)**、**超参数调优/学习率调度** |
| 簇E 模型生成 | **本期暂不覆盖，C期补充** |
...
| 模板 | 用途 |
|------|------|
| ml:template_diagnosis | 问题诊断（定位模型效果差的根因） |
| ml:template_model_selection | 模型选型推荐 |
| ml:template_code_generation | 代码生成 |
...
result = executor.invoke({"input": "为什么我的模型在测试集上表现很好但线上效果很差？"})
print(result["output"])
```
