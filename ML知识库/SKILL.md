...
| `ml:VerdictRule` | 判定规则（如：因果归因判定、合规判定） |
| `ml:TaskTemplate` | 任务模板（如：因果调试、RAG治理） |
...
# 识别
task = skill.recognize_task_type("为什么反欺诈模型误报率突然升高？")
assert task == "因果调试"

# 取模板
template = skill.get_template("ml:template_causal_debug")

# 填充槽位（必须提供error_signal和workflow_def）
filled = skill.fill_template(
    template=template,
    slots={
        "slot_error_signal": "反欺诈模型误报率突然从2%升至15%",
        "slot_workflow_def": {"agents": [...], "connections": [...]},
        "slot_agent_traces": [...]
    }
)
...
# report 包含：根因分析 + 因果图 + 修复建议 + 回归测试用例
print(report["root_cause"])    # Policy_Retrieval_Agent 误匹配
print(report["causal_graph"])  # 可视化路径图
print(report["fix_suggestions"]) # Prompt补丁 + 数据清洗建议
```
...
task = skill.recognize_task_type(
    "将合同文件夹进行向量化，保留违约条款上下文，人名和银行卡号脱敏"
)
template = skill.get_template("ml:template_rag_governance")

filled = skill.fill_template(
    template=template,
    slots={
        "slot_raw_data": "s3://company-confidential/2026-Q3-M&A-Deal/",
        "slot_nl_instruction": "保留违约责任和付款条款的完整上下文，人名和银行卡号脱敏"
    }
)

# 输出
print(filled["slot_semantic_slices"])      # 切片预览
print(filled["slot_desensitization_report"]) # 脱敏报告
print(filled["slot_pipefile"])             # 可执行Pipefile
```
...
```python
task = skill.recognize_task_type(
    "金融风控场景，反欺诈，强可解释性，延迟<20ms，CPU部署"
)
...
# 推荐：RandomForest（首选）/ LogisticRegression（备选）
```
...
| 模式 | 使用场景 | 实现方式 |
|------|---------|---------|
| 确定性代码 | 合规判定（A违反B条款→不合规） | `if/else` 条件表达式 |
| 双组件 | 因果归因（证据LLM抽→程序判决） | EvidenceExtractor + DeterministicVerdict |
| LLM评分+阈值 | 检索质量判定 | `{score, feedback}` + 阈值判定 |
...
| 决策 | 理由 | 实测修正 |
|------|------|---------|
| 因果调试一票规则权重0.75 | workflow_def缺失即阻断是防止无地图乱定位 | 实测：用户常漏填workflow_def，需在模板说明中强化前置说明 |
| RAG治理降级策略为"留空标注" | 语义碎分不一定致命，留空由用户判断 | 实测：会议记录等特殊格式需人工合并，策略正确 |
| 覆盖率阈值0.85（L3更严格） | 合规场景不容忍低覆盖率直接出稿 | 实测：有效防止不完整答案流出 |
